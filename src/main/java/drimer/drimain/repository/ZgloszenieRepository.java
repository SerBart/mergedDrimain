...
public interface ZgloszenieRepository extends JpaRepository<Zgloszenie, Long>, JpaSpecificationExecutor<Zgloszenie> {
    ...
    // NEW: fetch join to avoid LazyInitializationException
    @Query("select distinct z from Zgloszenie z " +
            "left join fetch z.autor " +
            "left join fetch z.dzial")
    List<Zgloszenie> findAllWithRelations();
